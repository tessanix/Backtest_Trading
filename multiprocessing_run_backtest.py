...
from strategy_runner_2 import Backtest
...
def run_strategy(combination, positionSize, instrument, start_date, end_date):
    ...
    stopMethod, nbrTimeMaxPassThroughTenkan, percentSlAlmostHit, slModifierAfterAlmostHit, ratioDistanceKijun, timeTPMovedToClose, calendar_events = combination 
    ...
    backtest = Backtest(strategy, us_calendar_df, instrument, onlyUSSession, feesPerTrade=4.36, positionSize=positionSize, forbiddenHours=[], 
            ...
            nbrTimeMaxMoveTP=nbrTimeMaxMoveTP, usSessionHour=sessionHour, stopMethod=stopMethod, nbrTimeMaxPassThroughTenkan=nbrTimeMaxPassThroughTenkan, percentSlAlmostHit=percentSlAlmostHit, slModifierAfterAlmostHit=slModifierAfterAlmostHit, timeTPMovedToClose=timeTPMovedToClose)
    trades = backtest.strategyLoop()
    ...
    result = (start_time.replace(microsecond=0), end_time.replace(microsecond=0), 
              [trades, onlyUSSession, timeframes, bracketsModifier, slInTicks, tpInTicks, tpToMoveInTicks, percentHitToMoveTP, nbrTimeMaxMoveTP, sessionHour, stopMethod, nbrTimeMaxPassThroughTenkan, percentSlAlmostHit, slModifierAfterAlmostHit, ratioDistanceKijun, timeTPMovedToClose, calendar_events])#chopv, chopp])
    return result
...
def main():
    ...
    params = {
        ...
        "timeTPMovedToClose":[1],
        "ratioDistanceKijun": [0.5],
        "slModifierAfterAlmostHit":[0.1],
        "percentSlAlmostHit":[0.65],
        ...
        "percentHitToMoveTP": [0.5],
        "tpToMoveInTicks": [2, 4, 6, 10],  #[2.5], 
        "bracketsModifier": [ 
            [[]],
            # [[0.85, 0.1]],   
            # [[0.85, -0.1]],   
            ...
            # [[0.6, -0.4], [0.7, -0.1], [0.8, 0.2], [0.92, 0.6]],
            # [[0.6, -0.5], [0.7, -0.1], [0.8, 0.3], [0.95, 0.65]],
            [[0.6, -0.5], [0.7, -0.2], [0.8, 0.3], [0.95, 0.5]],
        ],
        # "slInTicksBeforeSession": [40, 50],
        # "tpInTicksBeforeSession": [25, 45],
        # "slInTicksDuringSession": [55, 65],
        # "tpInTicksDuringSession": [65, 75],
        "slInTicksBeforeSession": [11],
        "tpInTicksBeforeSession": [6],
        "slInTicksDuringSession": [1],
        "tpInTicksDuringSession": [1],
        # "tickCrossed": [1,2,3],
        "onlyUSSession": [False],
    ...
    # for tickCrossed in params['tickCrossed']:
    for timeTPMovedToClose in params['timeTPMovedToClose']:
                                                                    ...
                                                                    for onlyUSSession in params["onlyUSSession"]:
                                                                        for timeframes in params["timeFrameUsed"]:
                                                                            comb = (onlyUSSession, timeframes, bracketsModifier, [slInTicksBeforeSession, slInTicksDuringSession], \
                                                                                    ...
                                                                                    sessionHour, stopMethod, nbrTimeMaxPassThroughTenkan, percentSlAlmostHit, slModifierAfterAlmostHit, ratioDistanceKijun, timeTPMovedToClose, calendar_events)
    ...
    filename = re.sub(r'[<>:"|?*]', '-', f"ES_m1_position={positionSize}_{datetime.now()}.pkl").replace(" ", '_')
    # print(path)
