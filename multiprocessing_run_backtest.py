...
def run_strategy(combination, queue, positionSize, instrument, start_date, end_date):
    iteration, onlyUSSession, smke, timeframes, tc, forbHours, slModifiers, tenkanAngle, slInTicks, tpInTicks, calendar_events = combination # windowForLevels nbr_of_points, delta_in_ticks = combination
    ...
    trades = strategyLoop(strategy, us_calendar_df, instrument, onlyUSSession, stopMethod=smke, feesPerTrade=1.42, 
                          positionSize=positionSize, forbiddenHours=forbHours, slModifiers=slModifiers, 
                          slInTicksInitial=slInTicks, tpInTicksInitial=tpInTicks)
    result = (iteration, [trades, onlyUSSession, smke, timeframes, tc, slModifiers, forbHours, tenkanAngle, slInTicks, tpInTicks, calendar_events])#chopv, chopp])
...
if __name__ == '__main__':
                                ...
                                for tc in params["ticksCrossed"]:
                                    for onlyUSSession in params["onlyUSSession"]:
                                        for timeframes in params["timeFrameUsed"]:
                                            comb = (iteration, onlyUSSession, smke, timeframes, tc, forbHours, slModifiers, tenkanAngle, slInTicks, tpInTicks, calendar_events)
    ...
    filename = f"size={positionSize}_timeframe={convertir_en_entiers(params['timeFrameUsed'])}_"\
        ...
        f"slModifiers={params["slModifiers"]}_usCalendar.pkl"
